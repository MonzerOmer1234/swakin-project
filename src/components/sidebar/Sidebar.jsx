...
export default function Sidebar({ tabs }) {
  ...
  return (
    <>
      ...
      >
        <a
          className="flex items-center text-lg text-blue-700 ps-2"
          href="#"
          aria-label="Brand"
        >
          <img
            ...
            alt="logo"
          />
          <p className="ms-5 flex flex-col justify-center items-start">
            <span className="text-xl font-bold">سواكن للنقل و التجارة</span>
            <span className="text-[10px]">Swakin Marina for Shipping & Trading</span>
          </p>
        </a>
          ...
          <ul className="space-y-1.5">
            {tabs && tabs.length > 0
              ? tabs.map((tab, index) => (
                  <li key={tab?.text}>
                    <Link
                      className={` flex items-center gap-x-3.5 py-2 px-2.5 text-gray-700 rounded-lg  dark:bg-neutral-700 dark:text-white ${
